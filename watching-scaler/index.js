...
const loop = () => {
  ...
  for (const schedule of schedules) {
    if (isTimeInSchedule(schedule, currentTime)) {
      ...
      const command = `kubectl -n ${namespace} scale deployment ${deployment} --replicas=${schedule.replicas}`;
      exec(command, (error, stdout, stderr) => {
        if (error) {
          log("error", `Failed to scale deployment ${namespace}/${deployment} to ${schedule.replicas} replicas`, { schedule, error, stderr });
        } else {
          log("info", `Successfully scaled deployment ${namespace}/${deployment} to ${schedule.replicas} replicas`, { schedule, stdout });
        }
      });
    }
  }
